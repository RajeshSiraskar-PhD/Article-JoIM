...
The in-house SIT study uses 20 evaluation rounds per held-out file to quantify variance from stochastic action sampling. The IEEE benchmark track is smaller in the present consolidated analysis. Because observations are nested within trained-model/held-out-file pairs, the inferential statistics are treated as exploratory secondary evidence and are interpreted together with matched held-out comparisons and cross-schema ranking consistency.
...
Prism-generated hypothesis outputs use a matched-cell design. For each schema, we ran omnibus repeated-measures ANOVA tests (\texttt{AnovaRM}) for Algorithm, Attention, and their interaction, with Friedman tests as nonparametric robustness checks. We then examined Holm-adjusted paired pairwise comparisons to localize the differences most relevant to H1--H3. Table~\ref{tab:hyp_tests} reports the \textsc{REINFORCE}-centered algorithm contrasts most relevant to H1. Because SIT additionally includes repeated stochastic rounds and the data remain nested by source file, these inferential results are treated as descriptive secondary evidence rather than as the sole basis for the paper's claims.

\begin{table}[htbp]
	\centering
	\caption{Holm-adjusted paired pairwise algorithm comparisons against \textsc{REINFORCE} (overall schema-level contrasts used to support H1).}
	\label{tab:hyp_tests}
	\begin{tabular}{l l r r l}
	\toprule
	\textbf{Dataset} & \textbf{Competitor} & \textbf{Stat.} & \textbf{Holm-adjusted $p$} & \textbf{Reject $0.05$}\\
	\midrule
	\multirow{3}{*}{SIT}
		& A2C & 0.0 & $7.14\times 10^{-16}$ & Yes\\
		& DQN & 0.0 & $7.14\times 10^{-16}$ & Yes\\
		& PPO & 1215.0 & 0.0010 & Yes\\
	\faintcmidrule{1-5}
	\multirow{3}{*}{IEEE}
		& A2C & 0.0 & $1.14\times 10^{-5}$ & Yes\\
		& DQN & 0.0 & $2.65\times 10^{-4}$ & Yes\\
		& PPO & 31.0 & 0.0200 & Yes\\
	\bottomrule
	\end{tabular}
\end{table}
...
Let $S$ denote the evaluation score (Section~\ref{sec:metrics}). Table~\ref{tab:hyp_tests} reports the overall algorithm-level pairwise comparisons against \textsc{REINFORCE}. The interpretation below combines these contrasts with the omnibus \texttt{AnovaRM}/Friedman outputs and the held-out ranking patterns across SIT and IEEE.
