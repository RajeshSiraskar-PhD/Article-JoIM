...
% Ver. 3.2 -- Dropped schema detection. Reverse loov
% ------------------------------------------------------------------------------------------------
...
We compare four RL algorithms (\textsc{A2C}, \textsc{DQN}, \textsc{PPO}, and \textsc{REINFORCE}) paired with four attention-style feature extractors (Nadaraya--Watson, temporal, multi-head, and self-attention) inside an industrial \emph{practioner-friendly} AutoRL workflow that standardizes training, repeated evaluation, and reporting. A Gymnasium-compatible environment is built around sensor measurements available during cutting and is designed to be schema-adaptive.

Across two different milling datasets (public IEEE PHM~2010 and an in-house SIT testbed), the adopted evaluation protocol shows \textsc{REINFORCE} as a strong lightweight baseline: it consistently outperforms \textsc{A2C} and \textsc{DQN} and remains close to \textsc{PPO}. Attention improves results, but the best attention mechanism is schema-dependent.}
...
The study addresses the following research objectives: (1) compare RL algorithms suitable for edge-oriented PdM, (2) evaluate whether \textsc{REINFORCE} is a competitive lightweight baseline, (3) assess attention mechanisms as a form of inductive bias that can improve lightweight RL, (4) design a Gymnasium-compatible RL environment that operates across differing sensor schemas, and (5) provide an AutoRL workflow for experimentation and training.
...
\begin{itemize}
	...
	\item \textbf{Practitioner convenience mechanism (AutoRL):} We build a practioner-friendly system. There is no hyperparameter tuning or feature engineering involved. Raw sensor readings are used. Batch training and evaluation with checkpointed recovery and standardized reporting to reduce the burden of manual tuning and ad-hoc experimentation.
\end{itemize}
...
Milling machines shape the work piece by rotating and driving a metal cutting device into it. The tool therefore experiences abrasive force that causes tool wear. In practice there is usally no tool maintenance procedures, and they are simply replaced on reaching end of useful life. ISO 8688-2:1989 suggests 300 $\mu$m as a standard wear threshold. We define this as $\tau$ and use it as a decision boundry.
...
Note that we use ``attention'' as an umbrella term for (i) learned feature reweighting/gating and (ii) structured fusion of sensor features and optional time context. In this work, attention operates on each timestep's sensor vector (feature selection/gating), not on sequence-wise ``token'' attention, as in natural-language processing (NLP); temporal context is injected only through explicit positional encoding.
...
Multi-head attention (MH) uses multiple heads to gate the single-timestep feature vector in parallel subspaces \cite{Vaswani2017}.
...
Self-attention projects the observation into a feature space and applies a scalar attention gate with residual connections and layer normalization.

In our implementation,
\begin{align}
	\mathbf{x} &= W_{\mathrm{in}}\mathbf{x}_t,\\
	s &= \frac{(W_Q\mathbf{x})^\top(W_K\mathbf{x})}{\sqrt{d}},\quad g=\mathrm{softmax}_b(s),\\
	\tilde{\mathbf{x}} &= g\,W_V\mathbf{x},\\
	\mathbf{z} &= \mathrm{LN}\!\left(\mathbf{x}+\tilde{\mathbf{x}}\right),\quad
\mathbf{z}=\mathrm{LN}\!\left(\mathbf{z}+\mathrm{FFN}(\mathbf{z})\right).
\end{align}
...
Hypothesis tests are performed with one-tailed Welch $t$-tests at $\alpha=0.05$ for each schema. Table~\ref{tab:hyp_tests} summarizes the results under two categories - overall and using the best performing attention mechanism.
...
Evaluations are performed on held out data and evaluation scores are computed per (model, held-out file) pair by averaging over the 20 stochastic rounds; reported $n$ values count these model--test-dataset pairs. Our evalaution strategy can be described as Reverse Leave-One-Out. We train on one full wear cycle dataset and evaluate how it performed on the rest. In some respects, this addresses the ``distributional'' bias bias described by \cite{George2025Evaluation}.
...
H2 is supported by the cross-schema consistency of the hypothesis-test outcomes: the same environment design and reward shaping yield a stable ranking signal in both SIT and IEEE, with \textsc{REINFORCE} significantly outperforming \textsc{A2C} and \textsc{DQN} and remaining close to (but still statistically above) \textsc{PPO}.
...
H2 is supported structurally by the environment design (Section~\ref{sec:env}) and empirically by consistent learning and ranking signals on both schemas (Tables~\ref{tab:hyp_tests}--\ref{tab:ieee_best_attn}). This should be interpreted as \emph{schema portability} of the experimental pipeline and MDP formulation---i.e., the same formulation produces meaningful learning and evaluation behaviour across heterogeneous sensor schemas---not as direct transfer of a trained policy from one schema to the other
...
This paper presented an empirical study of \textsc{REINFORCE} with attention mechanisms for milling-machine tool-replacement PdM under edge-oriented constraints. Using a schema-adaptive Gymnasium environment, the results indicate a consistent pattern across SIT and IEEE schemas: under the adopted evaluation protocol, \textsc{REINFORCE} emerges as a strong lightweight baseline, performing more strongly than \textsc{A2C} and \textsc{DQN} while remaining comparable to \textsc{PPO}. The study also shows that the same environment formulation and reward logic can be reused effectively across two materially different sensor schemas. Finally, attention is beneficial for \textsc{REINFORCE}, although the most effective choice is schema-dependent: multi-head attention is strongest on SIT, whereas several attention variants perform similarly on IEEE. Interpreted as representation-learning priors, the attention mechanisms bias the policy toward informative sensor patterns in an online monitoring setting.

\subsection{Future work}
As part of our progressive researc, we plan to study attention mechanisms on time-series windows - for example a 2 to 3 second window. This could prove to be significant 'Early Warning System'. Other limitations of our research that could be addressed (i) measure inference latency, serialized model size, and energy on representative edge devices, (ii) incorporate richer maintenance costs and state-estimation models, (iii) add non-RL baselines that operate on the same raw or minimally processed sensor streams, and (iv) evaluate robustness under sensor drift and cross-machine domain shift.
...
Conceptualization, methodology and implementation: Rajesh Siraskar. Milling machine set-up and data gathering: Satish Kumar. Manuscript review: Shruti Patil. Overall program management and funding: Ketan Kotecha and Ambarish Kulkarni
...
\begin{table}[htbp]
	...
	\begin{tabular}{@{} L{0.22\linewidth} L{0.14\linewidth} L{0.26\linewidth} L{0.14\linewidth} L{0.20\linewidth} @{}}
		\toprule
