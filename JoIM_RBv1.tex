...
\title[Lightweight RL for Edge PdM]{Lightweight Reinforcement Learning for Edge Predictive Maintenance: An Empirical Study of REINFORCE with Attention Mechanisms for Milling Tool Replacement}
...
The study considers the decision of when to replace a milling cutter under asymmetric costs: replacing too late risks quality loss or failure, whereas replacing too early wastes remaining tool life. We compare four RL algorithms (\textsc{A2C}, \textsc{DQN}, \textsc{PPO}, and \textsc{REINFORCE}) and four attention mechanisms (Nadaraya--Watson, temporal, multi-head, and self-attention) within an AutoRL workflow for standardized training, repeated evaluation, and consolidated reporting.

A Gymnasium-compatible PdM environment is developed with schema-adaptive sensor observations and partial observability, with tool wear hidden from the agent. Experiments on two milling datasets with different sensing modalities---the public IEEE PHM~2010 benchmark and an in-house university dataset---show that \textsc{REINFORCE} outperforms \textsc{A2C} and \textsc{DQN} under the adopted evaluation protocol and remains competitive with \textsc{PPO}. Attention mechanisms further improve performance, although the strongest attention variant depends on the dataset. Overall, the results identify \textsc{REINFORCE} as a lightweight baseline for edge-oriented PdM.}
...
Edge deployment introduces hard constraints on compute cycles, memory footprint, and energy consumption. These constraints motivate \emph{lightweight} learning methods and also connect to ``Green AI'' arguments: when many candidate agents are trained during sweep-based experimentation, the incremental cost per training run becomes operationally and environmentally relevant \cite{Schwartz2020, Strubell2019}.

\subsection{Problem statement}
This paper studies a prescriptive PdM decision for milling: \emph{when should a cutting tool be replaced?} The decision is cost-asymmetric: late replacement risks defective parts or failure, while early replacement discards remaining tool life. Reinforcement learning is a natural fit because the agent must trade off utilization against risk over time; in realistic deployments, the agent must do so under partial observability (tool wear is not directly observable) and under edge-oriented resource limits.

The core research question is: \emph{Can a computationally light policy-gradient algorithm such as \textsc{REINFORCE} remain competitive with more advanced deep RL methods (e.g., \textsc{PPO}) for milling PdM when paired with attention-based feature extraction?}
...
The study addresses the following research objectives (RO): (RO1) compare RL algorithms suitable for edge-oriented PdM, (RO2) evaluate whether \textsc{REINFORCE} is a competitive lightweight baseline (and when it is not), (RO3) assess attention mechanisms as a form of inductive bias that can improve lightweight RL under partial observability, (RO4--RO5) design a Gymnasium-compatible RL environment that operates across differing sensor schemas, and (RO6) provide an AutoRL-style sweep and reporting pipeline for standardized experimentation.

We test three hypotheses:
\begin{itemize}
        \item \textbf{H1 (primary):} A computationally light \textsc{REINFORCE} agent outperforms \textsc{A2C} and \textsc{DQN}, while remaining competitive with \textsc{PPO}, on the studied PdM task.
        ...
        \item \textbf{H3:} Attention mechanisms improve \textsc{REINFORCE} performance, with the strongest attention choice depending on schema; multi-head attention is strongest on SIT, while IEEE performance is shared across multi-head, self-attention, and temporal attention.
\end{itemize}
...
\subsection{Positioning lightweight RL for edge PdM}
...
\subsection{MDP notation and partial observability}
...
In the studied PdM setting, the agent does not observe tool wear directly; it instead receives a sensor vector $\mathbf{x}_t\in\mathbb{R}^d$ derived from multiple channels. This corresponds to partial observability and can be viewed as a POMDP where the observation $o_t$ is a function of the latent wear-related state. We therefore treat representation learning from sensor-only observations as a first-order factor in policy performance.
\label{sec:pomdp}
...
The principal architectural contribution of this study is the use of attention-based custom feature extractors $\phi(\mathbf{x}_t)$ that transform the raw sensor observation $\mathbf{x}_t$ into a 64-dimensional embedding (\texttt{features\_dim=64}) before it is passed to the otherwise standard multilayer-perceptron policy/value networks provided by Stable-Baselines3 (Appendix~\ref{app:attn_core_impl}).

\subsubsection{Attention mechanisms as lightweight feature extractors}
\label{sec:attention}
Because tool wear is not directly observed at decision time (Section~\ref{sec:pomdp}), the setting is partially observable in the standard RL sense and the core challenge becomes representation learning from sensor vectors. Attention is a representation-learning prior that biases the policy toward informative sensor patterns. This is particularly relevant in the present PdM setting, where tool wear is latent and decisions must be made from indirect sensor evidence. We implement attention mechanisms as feature extractors $\phi_\theta(\mathbf{x}_t)$ that map a raw sensor vector into a compact embedding used by the RL policy.
...
\subsection{Simulation environment: a Gymnasium-compatible milling PdM POMDP}
\label{sec:env}
...
\subsection{Ablation study / sensitivity analysis (H3): attention mechanisms}
...
The PdM task studied here has a small action space and terminates on replacement, but the decision is still sequential and partially observable because the agent never observes tool wear directly. Instead, it must infer replacement timing from multivariate sensor trajectories alone. In such settings, representation quality can matter at least as much as optimizer sophistication; this helps explain why a lightweight policy-gradient method paired with suitable feature extraction can compete with heavier baselines.
...
This paper presented an empirical study of \textsc{REINFORCE} with attention mechanisms for milling-machine tool-replacement PdM under edge-oriented constraints. Using a schema-adaptive Gymnasium environment and multi-round evaluation on unseen data, the results show a consistent pattern across SIT and IEEE schemas: \textsc{REINFORCE} outperforms \textsc{A2C} and \textsc{DQN} under the adopted evaluation protocol, and remains competitive with \textsc{PPO}. The study also shows that the same environment formulation and reward logic can be reused effectively across two materially different sensor schemas. Finally, attention improves \textsc{REINFORCE}, although the strongest attention choice is schema-dependent, with multi-head attention dominating on SIT and several attention variants performing similarly on IEEE. Interpreted as representation-learning priors, the attention mechanisms bias the policy toward informative sensor patterns when tool wear must be inferred from indirect sensor evidence.

\subsection{Limitations and threats to validity}
\begin{itemize}
        \item \textbf{Dataset-backed sequential environment:} the environment is sequential and partially observable, but it is still constructed from logged machining trajectories and models intervention through reward and episode termination rather than through downstream physical process changes; richer closed-loop maintenance dynamics remain for future work.
...
Future work should (i) measure inference latency, serialized model size, and energy on representative edge devices, (ii) incorporate richer maintenance costs and partial observability models, (iii) add non-RL baselines that operate on the same raw or minimally processed sensor streams, and (iv) evaluate robustness under sensor drift and cross-machine domain shift.
...
\section{Attention mechanisms core implementation detail}
\label{app:attn_core_impl}
This appendix makes explicit why attention is treated as the principal architectural contribution of the study rather than as an auxiliary add-on. In the implementation, attention is placed at the observation-encoding stage of the policy. The processing path is
\begin{equation}
\mathbf{x}_t \xrightarrow{\phi_\theta} \mathbf{z}_t \in \mathbb{R}^{64} \xrightarrow{\text{SB3 MLP policy/value heads}} \left(\pi(a_t\mid \mathbf{z}_t), V(\mathbf{z}_t)\right).
\end{equation}
Thus, the architectural novelty lies in how raw sensor vectors are transformed before reaching otherwise standard policy and value networks.

The evidence in the code is direct. First, the environment \texttt{MT\_Env} defines the observation space using only schema-specific sensor channels, while excluding \texttt{tool\_wear}, \texttt{Time}, and \texttt{ACTION\_CODE}. Consequently, the agent must infer replacement timing from indirect sensor evidence rather than from direct wear observations. This makes representation learning a first-order component of performance, not a secondary implementation choice.

Second, the attention modules are implemented as policy feature extractors rather than as external preprocessing blocks. In \texttt{rl\_pdm.py}, the study mechanisms \texttt{NadarayaWatsonExtractor}, \texttt{TemporalAttentionExtractor}, \texttt{MultiHeadAttentionExtractor}, and \texttt{SelfAttentionExtractor} all inherit from \texttt{BaseFeaturesExtractor}. In Stable-Baselines3 terms, this means that they replace the default observation encoder used by the policy (Refer to https://stable-baselines3.readthedocs.io/en/master/guide/custom_policy.html#custom-feature-extractor). Their role is therefore architectural: they determine what latent representation of the sensor vector is made available to the downstream actor and critic.

Third, the linkage from code to model architecture is explicit in the training setup. The \texttt{policy\_kwargs} dictionary selects the attention class through \texttt{features\_extractor\_class} and sets \texttt{features\_extractor\_kwargs=dict(features\_dim=64)}. The model is then instantiated as \texttt{AlgoClass("MlpPolicy", env, ... , policy\_kwargs=policy\_kwargs)}. Therefore, the downstream policy/value heads remain the standard Stable-Baselines3 multilayer perceptrons, while the attention-based encoder is the component that changes across configurations.

This separation is exactly why attention is described in the main text as the core implementation idea. The comparison isolates the effect of the learned encoder $\phi_\theta(\mathbf{x}_t)$ while keeping the policy/value heads standard. Listings~\ref{lst:nw_forward}--\ref{lst:sa_forward} further support this interpretation: each forward pass returns a learned context vector or projected latent feature representation, not an action. Actions are produced only after that representation is consumed by the standard policy network. Accordingly, attention functions here as the main inductive bias for extracting decision-relevant structure from partially observed sensor signals.
