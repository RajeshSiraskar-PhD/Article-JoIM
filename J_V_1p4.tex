...
\title[Lightweight RL for Edge PdM]{Lightweight Reinforcement Learning for Edge Predictive Maintenance: An Empirical Study of REINFORCE with Attention Mechanisms for Milling Tool Replacement (V.1.4)}
...
A significant quantitative study by Pandey et al. \cite{Pandey2023}, around edge deployment studies the memory bottleneck. They show that in a PdM edge-deployment study, pruning could reduce model sizes dramatically while keeping the average accuracy drop within about 3\%. For example, with Short-Time Fourier Transform (STFT) features, AlexNet was reduced from 28,534,479 to 75,627 parameters, resulting in an accuracy drop from 98.37\% to 97.65\%, while LeNet fell from 76,695 to 1,967 parameters--accuracy drop from 98.07\% to 97.13\%. With Continuous Wavelet Transform (CWT) features, AlexNet reduced parameters from 28,534,479 to 656,620. The model sizes for STFT and CWT, using AlexNet were both 28.18 MB, \cite{Pandey2023}. The implication of this research indicates that for DRL to be suitable for edge deployment, their deployment often depends on pruning, quantization, distillation, or related compression steps before edge feasibility can be discussed \cite{Pandey2023, Jang2020, Xu2022}.
\\
\\
\textcolor{red}{TinyRL: Towards Reinforcement Learning on Tiny EmbeddedDevices \cite{Szydlo2022}}\\
...
\begin{table}[htbp]
	...
	\begin{tabular}{@{} l l l l p{5.5cm} @{}}
		...
		\midrule
		STM32H7 Series & ARM & Dual-core Cortex-M7 + M4 & Up to 2 MB & Industrial powerhouse; M7 handles heavy ML math while M4 handles real-time sensor control. \\
		
		NXP i.MX RT Series & ARM & Cortex-M7 (up to 1 GHz) & 1 MB -- 2 MB & "Crossover" MCU; ultra-fast clock speeds for processing complex local anomaly detection. \\
		
		Espressif ESP32-S3 & Xtensa & Dual-core LX7 & 512 KB & Includes built-in Wi-Fi/BLE and CPU vector instructions to accelerate neural networks. \\
		
		Nordic nRF5340 & ARM & Dual-core Cortex-M33 & 512 KB & Ultra-low-power BLE gold standard; dedicated separate cores for application and wireless stack. \\
		
		Renesas RA6M5 & ARM & Cortex-M33 & 512 KB & Ideal for industrial motor control; features robust security, Ethernet, and CAN bus controllers. \\
		
		Raspberry Pi RP2040 & ARM & Dual-core Cortex-M0+ & 264 KB & Highly cost-effective; features unique Programmable I/O (PIO) blocks for custom sensor interfacing. \\
		
		Nordic nRF52840 & ARM & Cortex-M4F & 256 KB & Includes an FPU for crucial vibration math; widely used in battery-operated nodes. \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{table}[t]
	\centering
	\caption{SIT schema (averaged over 128 models)}
...
\subsection{Algorithm description}\label{sec:algo_desc}
\subsubsection{AutoRL as a practitioner convenience: standardized sweeps and reporting}\label{sec:autorl}
